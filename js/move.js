...
(function(){
    ...
    function  move(obj,target,opt){
        opt=opt||{};//这里为了防止对象.属性名报错的问题；
        var effect=opt.effect;//运动效果
        var callback=opt.callback;
        var tmpEffect=effect||zhufengEffect.Linear;
        var ary = ["Linear", "Elastic-easeOut", "Back-easeOut", "Bounce-easeOut", "Expo-easeIn"];
        //对运动形式做各种判断
        if(typeof effect==='number'){//如果effect传的是数字类型，说明，他想要的是数组中的运动形式；
        ...
        for(var attr in target){
            begin[attr]=utils.css(obj,attr);//获取非行间样式
            change[attr]=target[attr]-begin[attr];
        }
        var duration=opt.duration||1000;
        var time=null;
        //开启定时器，让时间累加，求出最新的位置，同时，赋值最新的位置；
        ...
        obj.timer=setInterval(function(){
            ...
            for(var attr in change){
                ...
                utils.css(obj,attr,curPos);//设置一个最新位置
            }
        },10)
